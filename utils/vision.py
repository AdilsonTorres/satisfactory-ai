...
class Vision:
    ...
    def capture(self) -> np.ndarray:
        """Captures the current frame. ~1ms, with a fallback to 'import' on Linux."""
        try:
            raw = self.sct.grab(self.monitor)
            frame = np.array(raw)
            return cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
        except mss.exception.ScreenShotError:
            # Fall back to ImageMagick's 'import' utility if mss fails
            import subprocess
            import tempfile
            import os

            # Try to find the Satisfactory window ID
            ...
            try:
                ...
                if frame is None:
                    raise RuntimeError("Failed to read image generated by 'import'")
                return frame
            finally:
    ...
    def capture_region(self, x: int, y: int, w: int, h: int) -> np.ndarray:
        try:
            ...
            # On failure, capture the full screen (already with fallback) and crop in memory
            frame = self.capture()
