...
# Desabilita o delay padrão do pydirectinput (nós controlamos o timing)
...
def hold(key: str, duration: float):
    """Segura uma tecla por X segundos (ex: mover para frente)."""
...
def click(x: int, y: int, button: str = "left", delay_after: float = 0.1):
    """Clica em coordenadas absolutas de tela."""
...
def move_mouse_relative(dx: int, dy: int):
    """
    Move o mouse de forma relativa — essencial para câmera 3D.
    pydirectinput.move() usa movimentos relativos compatíveis com Raw Input.
    """
    pydirectinput.move(dx, dy, relative=True)


def aim_at_screen_position(target_x: int, target_y: int,
                            screen_center_x: int, screen_center_y: int,
                            sensitivity_factor: float = 1.0):
    """
    Move a mira para um ponto na tela.
    sensitivity_factor: ajuste baseado na sua sensibilidade in-game. Calibre empiricamente.
    """
...
def interact():
    """Tecla padrão de interação do Satisfactory."""
...
def dodge(direction: str = "a"):
    """Dodge rápido para desviar de ataque."""
    hold(direction, 0.15)


def loot_remains():
    """
    Sequência de loot de cadáver:
    Olha levemente para baixo (remains ficam no chão), pressiona E.
    """
